  ...
  <div>
    ...
    <div class="flex flex-col gap-[8px]" v-if="messageStore.isError">
      ...
      <span class="flex flex-row-reverse justify-between flex-wrap h-fit flex-grow-0 flex-1">
        <p class="text-[12px] text-right text-[#FF0944] mr-2 cursor-pointer" @click="retryFunction">
          Thử lại
        </p>
      </span>
    </div>
    ...
    <div class="text-md mb-2 h-fit w-fit ml-auto py-4 pl-4 mr-4" v-else>
      <div
        class="bg-orange-500 rounded-tl-2xl rounded-tr rounded-l-2xl rounded-b-2xl text-md mb-2 h-fit w-fit text-[16px] p-4 break-words max-w-full sm:max-w-[338px] font-sf-pro-display mr-4">
        <p class="text-white text-right">{{ message }}</p>
      </div>
      <span class="flex flex-row-reverse justify-between flex-wrap h-fit font-sf-pro-display mr-4">
